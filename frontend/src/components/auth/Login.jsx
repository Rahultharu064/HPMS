...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (token && user) {
      try {
        ...
        // Redirect to returnUrl if present, otherwise to profile
  ...
  const handleGoogleLogin = () => {
    ...
    try {
      // Get returnUrl from current URL params
      const urlParams = new URLSearchParams(location.search);
      const returnUrl = urlParams.get('returnUrl');

      // Redirect to Google OAuth with returnUrl
      const apiUrl = import.meta.env.VITE_API_BASE_URL;
      const googleAuthUrl = `${apiUrl}/api/auth/google`;
      const finalUrl = returnUrl ? `${googleAuthUrl}?returnUrl=${encodeURIComponent(returnUrl)}` : googleAuthUrl;
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-white/50">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Access to member-only rates and special offers</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-white/50">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Manage bookings and preferences effortlessly</p>
                </div>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="w-full inline-flex justify-center py-3 px-4 border border-gray-300 rounded-xl shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {googleLoading ? (
                      <div className="flex items-center gap-2">
